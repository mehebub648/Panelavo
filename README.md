...
| Docker          | One supported root Compose file, a private rootless daemon and image store per site user, entry-service/container-port detection, loopback port mapping and verification, safety validation, status/logs, lifecycle actions, and explicit two-phase migration from legacy rootful projects. |
