...
| Docker          | One supported Compose file (in the application root or a discovered subfolder such as `docker/`), a private rootless daemon and image store per site user, entry-service/container-port detection, loopback port mapping and verification, safety validation, status/logs, lifecycle actions, and explicit two-phase migration from legacy rootful projects. |
