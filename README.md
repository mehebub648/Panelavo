...
The application cookie is opaque, `HttpOnly`, `SameSite=Strict`, scoped to `/`, and `Secure` on the public HTTPS route. Super Admins configure session lifetime and password rules in Settings; `SESSION_MAX_AGE_SECONDS` seeds the lifetime until it is saved there. Sessions are mirrored to the protected `.data` directory so normal process reloads do not sign users out. Profile lists active devices and can revoke individual or all other sessions, while a password change automatically revokes every other session. Login and mutation throttling is also persisted there, so a restart does not clear abuse counters. Production still runs one Next.js process because session and mutation serialization are single-process concerns.
...
Managers and Super Admins can open `/resources` without waiting for the privileged resource snapshot: the route renders first and fills in the live measurements separately. Website rows support search, runtime filtering, active-only filtering, sorting, and pagination. CPU is sampled per process; memory uses proportional set size (PSS) when available so shared pages are divided instead of counted repeatedly; each website disk value covers only its configured application root and explicitly marks roots shared by multiple website records. Opening Disk details starts a separate low-priority, cached filesystem analysis so the normal resource poll stays fast. That analysis accounts for complete site-user homes (including rootless Docker stores, backups, and caches), other home users, operating-system files, service data, administrator data, temporary files, filesystem overhead/unclassified allocation, and filesystem-reserved capacity. Site-user drill-downs include Docker-reported image, container, volume, build-cache, and reclaimable figures with an overlap warning for shared layers. A Super Admin can run a confirmed, serialized cache reclaim from this view: Panelavo processes each private site-user daemon in turn, retains up to 5 GB of BuildKit cache per site, and never prunes containers, images, volumes, databases, backups, or application files. Independently, a boot-time fifteen-minute storage guard begins bounded cleanup at 75% usage, tightens only at 90%, and pauses storage-growing actions at 92% or when the filesystem reserve is low. It removes disposable rootless build cache and, only in emergency mode, images older than 30 days that no running or stopped container references; it never prunes containers, volumes, databases, backups, or application files. Trusted setup also installs PM2 log rotation, and each newly initialized rootless Docker daemon defaults to five 20 MB local log files unless the site declares an external logging driver or a stricter Compose policy. The live process snapshot polls every thirty seconds while visible and shares a twenty-five-second server cache, reducing privileged `/proc` scanning and transient allocation across concurrent viewers. Panelavo attributes site-user, rootless-container, root-Docker, and root-owned application processes only when Unix ownership, cgroup, application-path, Compose, or upstream-port evidence identifies one website. Shared or unresolved work remains in separate Shared and System totals rather than being divided or guessed; for example, a common database engine stays System when the operating system cannot prove which website caused its current work.
...
### Private databases and public endpoints

The main MySQL/MariaDB service is loopback-only. A site writer may expose one selected database from its Databases tab; Panelavo creates a dedicated CloudPanel database user whose grants cover only that schema, assigns a unique `db-<label>.<server-ip>.<base-domain>` hostname and port, requires TLS, and shows the generated password once. Read-only plus an IP/CIDR allowlist is the default; internet-wide and read-write access require an explicit choice and exact-hostname confirmation. ProxySQL enforces TLS, schema lock, per-endpoint port binding, and a 20-connection credential limit, while Nginx Stream enforces the allowlist and per-IP connection limit. Revocation closes the listener, kills active gateway sessions, removes the ProxySQL credential, and deletes the dedicated CloudPanel user before the database is private again. The feature is browser-session-only and requires the current Panelavo password; API tokens and MCP cannot expose a database.

`setup.sh` uses a supplied wildcard certificate when it covers the database suffix. Otherwise it creates a private Panelavo database CA; clients must download that CA from the one-time credential screen and enable hostname verification. Raw ports 3306 and 33060 remain denied, and inactive gateway ports have no listener.
...
The available tools follow the live role: users can inspect assigned websites; site writers can create/update sites, manage supported site sections, domains/DNS/SSL, project endpoints, uptime checks, deployment hooks, scheduled and off-site backups, deployments, allow-listed Operations, the site-user Terminal, managed releases, proxy diagnosis, contained ACL repair, private rootless-runtime restart, and deletion; managers can also inspect server resources; Super Admins can perform host-level website and journaled migration recovery only when the bridge confirms their live CloudPanel role. Routine site-user terminal commands, deployments and non-destructive file-manager work run after live website-write authorization without another MCP prompt. Other sensitive changes ask the signed-in user for a separate, short-lived confirmation bound to that exact tool and its arguments; deletion also requires the exact domain. Account security, Panelavo account administration, panel settings, and the browser-only one-time phpMyAdmin sign-in remain UI-only.
...
Long MCP deployments, backups, and allow-listed Operations can run as background jobs. Starting a job returns its ID immediately; status calls expose bounded lifecycle logs, result summaries, explicit failure and timeout state, and honest interruption state after a Panelavo restart. A resolved server operation is still recorded as failed when its aggregate run or any reported step has a nonzero exit code or times out, while retaining the bounded result for diagnosis. Jobs are bound to the originating MCP credential and current writable-site access, limited to one active job per website, retained for seven days (up to 100 per actor), and time out within 30 minutes. Cancelling a job sends an abort signal through the shared site service and stops the complete privileged broker process group so child builds cannot continue invisibly. Existing synchronous tools remain available for compatible clients.

Completed tar.gz artifacts can be deployed through managed versioned releases for Node.js, static-build, PHP, and Python plans. Panelavo revalidates the artifact checksum inside the root broker, rejects unsafe archive entries and archives that exceed 10,000 entries or the bounded unpacked-storage allowance, preserves existing managed environment files, checks required paths, and validates the selected server-owned plan before changing the live root. The application root becomes one atomic symlink pointer; the original directory is retained as the first rollback release. Deployment and a caller-selected public HTTPS health path must both pass, otherwise Panelavo swaps back and redeploys the previous release automatically. A one-command rollback has the same health-gated recovery behavior. At most ten site-user-owned release trees are retained. Compose artifacts are intentionally rejected here because their bind mounts and datastores require the separate data-aware deployment contract.

For rootless Compose sites that keep LanceDB in a contained bind directory such as `data/lancedb`, MCP can inventory tables and create selective snapshots with include/exclude globs. Panelavo briefly stops only that site's Compose project, archives complete validated `.lance` table trees with a SHA-256 manifest under the site user's `backups/datastores` folder, and restarts through the normal port and readiness gates. A selective restore swaps whole table directories only after checksum and structure validation, restarts the application so cached LanceDB handles are refreshed, and can require numeric fields such as `directory_donors` from a same-site JSON endpoint. A failed readiness or data-count check restores the previous tables automatically. Because interrupting a table swap could strand the site between states, snapshot and restore jobs cannot be cancelled after they start. This workflow is table-selective and is not a substitute for a complete application/database backup.
...
The browser UI and MCP share actor-aware services for core sites, sections, domains, project endpoints, and site automation; the scoped `/api/v1` surface shares the core section service. Those boundaries retain live assignments, schema validation, endpoint ownership and health policy, exact server-owned plans, application-root containment, the unprivileged site-user boundary, operation locks, and root-broker checks.
...
The website list is grouped in the configured Project-category order, with external or legacy sites under **Uncategorized** and active project endpoints immediately below their parent. An optional private label (up to 80 safe characters) can be set during creation or in website Settings. Creation uses the authoritative site-record ID returned by CloudPanel, and labels are stored against both that ID and the normalized domain, so a recreated domain never inherits an old label. Runtime, site user, status, and creation date remain available in the website-specific Settings details card rather than the main list.
...
Customer-entered domains are aliases: the Domains tab and create form add them to the vhost `server_name`, point DNS through the user's connected Cloudflare token when it manages the zone, and issue Let's Encrypt certificates covering selected domains. Manual issuance includes the system domain and exactly the selected aliases; it does not silently add apex/www companions. Site creation, SSL preparation, and the explicit DNS action share one A-record policy for the primary name and applicable `www` companion. The system subdomain can be blocked with 403 or redirected to an alias; ACME challenge paths stay reachable so renewals keep working.
...
Each website has two path settings. **Root directory** is the application workspace used by Git, File Manager, Terminal defaults, Environment, Operations, cron working-directory guidance, and backups. **Serving directory** is CloudPanel's NGINX document root. For a PHP or static repository whose public entry point is in `public`, set the root to the repository directory and the serving directory to `<repository>/public`. Both paths are relative to the site user's `htdocs` directory. Existing sites use their current serving directory for both roles until a separate root is saved.

The Git section can initialize an existing site root or clone into an empty one. For a newly created PHP site, Panelavo records a root-owned fingerprint of the small initial CloudPanel scaffold; clone may replace only those exact unchanged files. The repository is fully fetched into a temporary child before the scaffold is moved aside, and any failed promotion restores the original files. An edited scaffold, an extra file, a directory, or any other non-empty root remains blocked. The ACME-managed `.well-known` directory is preserved while the repository is checked out. SSH clones use the site user's deployment key, accept and persist a previously unseen host key on first connection, and run non-interactively. Add the public deployment key shown beside repository connection or under Advanced Git tools to private repositories before cloning. The working tree lists changed files individually; selecting one opens an on-demand side-by-side diff. Operators can discard one file or all uncommitted work after confirmation. Updating source files requires a clean working tree, including untracked files, and a fast-forward update to the exact fetched branch commit. Panelavo does not automatically stash files; existing recovery stashes remain untouched. An optional after-pull plan runs up to ten ordered, existing allow-listed Operations under the same per-site lock as the pull. The plan is stored server-side, re-resolved against current preflight state, accepts only narrow script/process identifiers when an operation requires one, and stops at the first failure; it is never a free-form deployment shell. Repository operations have a five-minute limit and return actionable errors for other non-empty roots, authentication failure, and an invalid repository or branch.

**Deploy latest changes** updates source files, applies the detected plan or saved **Custom deployment steps**, and checks the application response. **Update files only** skips deployment steps and can immediately affect PHP/static sites. **Deploy current files** explicitly includes local modifications. Ordinary deployments update the current application directory and may interrupt traffic; they do not promise automatic rollback. Migrations are excluded from detected plans; existing custom migrations remain visible.

Browser deployments return a saved job immediately, retain failed-step output after navigation, and mark interrupted work after a panel restart without replay. Health requires HTTP 200–399 at the relative health path (default `/`) and required Compose services running/healthy within 90 seconds. History lasts seven days, up to 100 jobs per actor, with bounded, redacted output and truncation markers. Deployment cancellation is not offered.

Automatic deployment is opt-in under deployment settings. Create a website-specific `deployments:write` token (shown once, 90-day default expiry), then POST `{ "branch": "main", "expectedCommit": "<full tested commit>" }` with bearer authentication and an `Idempotency-Key` to `/api/v1/sites/<domain>/deployments`. Poll `/api/v1/sites/<domain>/deployments/<id>`; every final status except `succeeded` must fail CI. Reject stale tested commits. These tokens cannot access unrelated APIs or another website. The UI provides GitHub Actions and generic CI examples; Panelavo does not host a runner.

Every managed application root also carries an inherited POSIX ACL for its CloudPanel site user. Panelavo initializes the ACL across existing paths when the workspace is first accessed and verifies it before section mutations, so files written by Docker containers or other runtime UIDs remain readable, writable, and traversable by the site user. This preserves inode ownership and does not make the project world-writable.
...
A single application often exposes several HTTP processes on different ports — an API, an auth service, or a websocket gateway. A site writer can manage these from the parent website's **Project endpoints** card or the equivalent MCP tools. Panelavo lists only high loopback listeners that the root bridge can prove belong to the parent site's Unix user or application tree; wildcard, public, foreign, and CloudPanel-reserved ports are ineligible. Endpoint access is inherited from live write access to the parent project rather than granted as a separate per-port role.

A stopped service can be saved as a private pending reservation without creating a CloudPanel website or public proxy. Verification activates it only after the listener passes ownership and a bounded HTTP check at `/`; responses below HTTP 500 count as reachable so authenticated and not-found roots can still be proxied. An active endpoint is a normal CloudPanel reverse-proxy site with its own system domain, aliases, and SSL. Port changes are preflighted and checked again after the NGINX update; failure restores the previous upstream automatically. Child-site settings and raw vhost editing cannot bypass this gate. Deleting an endpoint removes only its reservation or child proxy, while deleting a parent remains blocked until all endpoints are removed. The older linked-service MCP tools remain compatible aliases for listing and creating endpoints.
...
| Docker          | One supported Compose file (in the application root or a discovered subfolder such as `docker/`), a private rootless daemon and image store per site user that a site-write user can self-initialize once `setup.sh` has provisioned the host, entry-service/container-port detection, loopback port mapping and verification, safety validation, status/logs, lifecycle actions, and explicit two-phase migration from legacy rootful projects. |
...
Docker sites use an independent rootless daemon owned by the CloudPanel site user. Panelavo passes its exact Unix socket for every non-interactive command and never retries against the host daemon. The rootless setup tool's Docker context supports direct SSH `docker` commands; Panelavo does not add a competing `DOCKER_HOST` profile export. Ordinary lifecycle actions are available to site-write users because they expose only what the same Unix account can do through SSH. Initialization, host packages, rootful migration/recovery, and legacy cleanup remain explicitly confirmed Super Admin operations. No account is added to the root-equivalent [`docker` group](https://docs.docker.com/engine/install/linux-postinstall/).

The Operations readiness panel checks the per-user systemd manager and D-Bus, linger, subordinate UID/GID ranges, socket, rootless security mode, storage/cgroup/network support, image-store usage, reclaimable data, and filesystem space. Images and volumes live under `/home/<site-user>/.local/share/docker`, so base images are duplicated between sites. Userspace port forwarding may make a proxy address appear as the immediate container peer; applications should trust CloudPanel's configured forwarded headers, not use that peer IP for authorization. Published ports below 1024 are unsupported.

Legacy rootful projects migrate in two confirmed phases. **Prepare** pulls or builds one service per request in the private daemon while the live project keeps serving; changes to Compose, images, containers, subordinate ranges, or bind ownership invalidate the expiring manifest. Named/external volumes, out-of-root paths, unsupported features, and ambiguous or conflicting owners block migration. **Cut over** stops but retains rootful containers, journals ownership, maps root-owned files to the site UID/GID and a container id `n >= 1` to `subordinateStart + n - 1`, reapplies named/default site-user ACLs, starts only prepared images with `--no-build`, and verifies containers, health, loopback ports, bind access, and HTTP. A failure restores ownership and ACLs and restarts the retained rootful project; an incomplete journal remains a visible recovery blocker.
...
Each website has a Backups tab that creates on-demand or scheduled, on-server snapshots: a gzip tar of the application root plus a `clpctl` gzip export of each selected database, written under `/home/<site-user>/backups/<id>/` with a manifest. Daily or weekly schedules run at a selected UTC hour from the single Panelavo process and retain the newest 1–100 snapshots. A backup is created atomically — any failed step removes the partial snapshot. Restore extracts the archived files over the live tree and imports each database back into its existing CloudPanel database; delete removes a single snapshot. Backups take the same per-site lock as Operations, so two heavy jobs cannot overlap on one website; a scheduled run records a skipped outcome when that lock is busy. On-demand jobs run synchronously; `setup.sh` configures the panel's Nginx proxy to keep their bounded requests open. Snapshots are browsable and downloadable from the File Manager (open the `backups` folder) or over SSH/SFTP for large archives.

An optional per-site S3-compatible destination streams a complete snapshot bundle to S3, Backblaze B2, Cloudflare R2, or a compatible HTTPS endpoint after each successful scheduled backup. Operators can also copy an existing local snapshot, list and delete remote copies, or restore one. Restore is local-first: Panelavo downloads the bundle into a private runtime staging directory, validates and imports it into `/home/<site-user>/backups/<id>/`, then uses the existing restore path. Access keys are AES-256-GCM encrypted in `.data/backup-destinations.enc`; secret keys are never returned to the browser.
...
The existing one-minute resource sampler evaluates panel-wide CPU, memory, and root-disk thresholds by default, with disk warning at 75%. A threshold must remain breached for the configured number of samples before an alert is sent; the same debounce applies before a recovery notice. Each website can enable a 1–60 minute HTTPS uptime check from its Settings tab. The sites list shows the last persisted up/down state, and repeated failures plus recovery produce notifications. Enabled checks also inspect the live TLS certificate every six hours and warn before the configured expiry window. Panel update availability and host security-maintenance state are checked every six hours, storage protection checks every fifteen minutes, and scheduled backup failures notify immediately. Monitoring configuration and state survive restarts in `.data/monitoring.json`.
...
Super Admins can check and install Panelavo updates from Settings. `setup.sh` seeds the public HTTPS update repository from `PANEL_UPDATE_REPOSITORY` or the checkout's compatible HTTPS `.git` origin; it can be changed later and is persisted in `.data/panel-settings.json`. Without an explicit or inferred repository, Settings remains usable and asks the operator to configure one before checking. An update check reads the candidate release version, commit, and required broker protocol, verifies the installed broker, and offers installation only when the repository contains a strictly newer stable version with an exactly compatible broker. An older repository is shown as behind the installed server, while a different commit that reuses the installed version is blocked as a release error. Panelavo clones an accepted release into a private staging directory, repeats the version-direction and broker checks, installs the locked dependencies, and requires a successful production build before synchronizing the release. `.data` and `.env.local` are preserved. Only the `panelavo` PM2 process reloads, so managed websites are not restarted.
...
Never grant raw PHP, `clpctl`, `bash`, `sh`, or unrestricted sudo to the panel service user. The broker and bridge are root-owned, accept no command-line arguments, validate a versioned JSON protocol, and cannot be replaced by self-update. Trusted setup also overrides CloudPanel's stock all-user `clpctlWrapper` grant for every non-service Unix user, so a website Terminal or SSH account cannot turn that wrapper into host administration; Panelavo's broker remains the only privileged path.
...
- True release-directory zero-downtime deployments and staging/clone environments remain roadmap items. Both require an explicit design around CloudPanel's authoritative document root; the current after-pull plan operates in place.
