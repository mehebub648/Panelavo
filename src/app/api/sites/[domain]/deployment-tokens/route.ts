...
import { manageDeploymentTokens } from "@/server/deploy/deployments";
...
export async function GET(
  ...
  try {
    ...
    return ok(await manageDeploymentTokens(actor, domain));
  } catch (error) {
    return fail(error);
  }
}
export async function POST(
  ...
  try {
    ...
    const result = await manageDeploymentTokens(
      actor,
      domain,
      await request.json(),
    );
    await audit("site.deployment-tokens.updated", "success", {
      actor: actor.user,
