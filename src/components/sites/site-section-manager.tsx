...
} from "@/components/sites/file-manager";
...
export function SiteSectionManager({
  ...
  async function act(input: Record<string, unknown>, skipConfirm = false) {
    ...
    if (
      ["delete", "clear"].some((value) =>
        String(input.action).includes(value),
      ) &&
      !skipConfirm
    ) {
  ...
  async function readLog(name: string) {
    ...
    try {
      const response = await fetch(
        `${apiBase}/api/sites/${encodeURIComponent(domain)}/sections/logs`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
      ...
      const result = await response.json();
      if (!result.success)
        throw new Error(result.error?.message || "Could not read this log.");
      setLogViewer(result.data);
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "Could not read this log.",
      );
    } finally {
      setBusy(false);
    }
  ...
  function renderContent() {
        ...
        <div className={card}>
          <form
            onSubmit={async (event) => {
              ...
              if (saved) setSavedEditorContent(editorContent);
            }}
            className="space-y-4"
          >
            ...
            <CodeEditor
              ...
              language="nginx"
              height="62vh"
              ariaLabel="NGINX configuration"
            />
            {feedback}
            <div className="flex items-center justify-between gap-3">
              <p
                className={`flex items-center gap-2 text-sm font-medium ${editorContent === savedEditorContent ? "text-emerald-600" : "text-amber-600"}`}
              >
                {busy ? (
                  <>
            ...
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {allowLocalSecrets
                    ? "Manage databases and open them in phpMyAdmin — Manage signs you in as the database's own user automatically."
                    : "Manage databases. Public exposure and one-time phpMyAdmin access remain local to this Node."}
                </p>
              </div>
              <div className="flex gap-2">
                {allowLocalSecrets && databaseManagerUrl && (
                  <Button asChild variant="outline" size="sm">
                    <a
                      href={databaseManagerUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <ExternalLink className="h-4 w-4" /> Open phpMyAdmin
                    </a>
                  </Button>
                )}
                <Button
                  size="sm"
                  onClick={() =>
                    setOpenForm(openForm === "database" ? null : "database")
                  }
                >
            ...
            <div className="space-y-3">
              {data.gateway && !data.gateway.ready ? (
                <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                  Secure database exposure is unavailable until a Super Admin
                  runs trusted setup. Existing endpoints remain fail-closed and
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    ...
                    {allowLocalSecrets && databaseManagerUrl && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => void openDatabase(item.name)}
                      >
          ...
          {openForm === "database" && (
            ...
            >
              <form
                className="space-y-4"
                onSubmit={async (e) => {
                  if (await submit("add", e)) setOpenForm(null);
                }}
              >
                ...
                <div>
                  ...
                  <Input
                    ...
                    title="Start with a letter; use only letters, numbers, and hyphens"
                    defaultValue={`${domain.split(".")[0].replace(/[^a-z0-9]/gi, "-")}-db`}
                    required
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    title="Start with a letter; use only letters, numbers, and hyphens"
                    defaultValue={`${domain.split(".")[0].replace(/[^a-z0-9]/gi, "-")}-user`}
                    required
                  />
                </div>
                <div>
          ...
          <section className={card}>
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <Button
                size="sm"
                onClick={() =>
                  setOpenForm(openForm === "certificate" ? null : "certificate")
                }
              >
            ...
            <div className="space-y-3">
              {(() => {
                ...
                return validItems.map((item) => (
                  <div
                    key={item.id}
                    className={`group flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4 transition-all hover:shadow-sm ${item.default ? "border-emerald-200/70 bg-emerald-50/40" : "border-slate-200/60 bg-white/50 hover:bg-white"}`}
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <KeyRound
                          className={`h-4 w-4 ${item.default ? "text-emerald-600" : "text-red-500"}`}
                        />
                    ...
                    {item.default ? (
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        <Button
                          ...
                          disabled={busy}
                          onClick={() =>
                            act({ action: "set-default", id: item.id })
                          }
                        >
                          Activate
                        </Button>
        ...
        <div className="grid gap-5 md:grid-cols-2">
                ...
                {((data[x.key] as string[] | undefined) ?? []).map((v) => (
                  ...
                  >
                    ...
                    <button
                      aria-label={`Remove ${v}`}
                      className="opacity-60 transition-opacity hover:opacity-100"
                      onClick={() => act({ action: x.del, value: v })}
                    >
                      <Trash2 className="h-4 w-4 text-red-500" />
                    </button>
                  </div>
                ))}
                {!((data[x.key] as string[] | undefined) ?? []).length && (
                  <div className="rounded-xl border border-dashed border-slate-200 py-7 text-center text-sm text-slate-400">
          ...
          <section className={card}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3">
                <span
                  className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${data.basicAuth?.active ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}
                >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-semibold ${data.basicAuth?.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                    >
                      {data.basicAuth?.active ? "Enabled" : "Disabled"}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-slate-500">
                    Require a username and password before serving the site.
                  </p>
                  {data.basicAuth?.active && (
                    <p className="mt-2 text-xs text-slate-400">
                      User: {data.basicAuth.username}
                    </p>
                  )}
                </div>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setOpenForm("basic-auth")}
              >
                Configure
              </Button>
            </div>
          ...
          <section className={card}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3">
                <span
                  className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${data.cloudflareOnly ? "bg-blue-50 text-blue-600" : "bg-slate-100 text-slate-500"}`}
                >
                  <Cloud className="h-5 w-5" />
                </span>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="font-bold">Cloudflare-only traffic</h2>
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-semibold ${data.cloudflareOnly ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-500"}`}
                    >
                      {data.cloudflareOnly ? "Enabled" : "Disabled"}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-slate-500">
                    Reject requests that do not originate from Cloudflare&apos;s
                    published networks.
                  </p>
                </div>
              </div>
              <button
                type="button"
                role="switch"
                aria-checked={Boolean(data.cloudflareOnly)}
                disabled={busy}
                onClick={() =>
                  act({ action: "cloudflare", enabled: !data.cloudflareOnly })
                }
                className={`relative h-7 w-12 shrink-0 rounded-full transition-colors ${data.cloudflareOnly ? "bg-panel-600" : "bg-slate-300"}`}
              >
                <span
                  className={`absolute left-1 top-1 h-5 w-5 rounded-full bg-white shadow transition-transform ${data.cloudflareOnly ? "translate-x-5" : "translate-x-0"}`}
                />
              </button>
            </div>
          </section>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <section className={`${card} md:col-span-2`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <KeyRound className="h-5 w-5 text-panel-600" />
                  <h2 className="font-bold">Deployment key pair</h2>
                </div>
                <p className="mt-1 text-sm text-slate-500">
                  Use this public key with GitHub, GitLab, or another remote
                  server.
                </p>
              </div>
              {!data.keyPair?.exists && (
                <Button
                  disabled={busy}
                  onClick={() => act({ action: "generate-keypair" })}
                >
                  {busy ? (
                    <LoaderCircle className="h-4 w-4 animate-spin" />
                  ) : (
            ...
            {data.keyPair?.exists ? (
              <div className="mt-5 grid gap-4 lg:grid-cols-2">
                <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
                  ...
                  <code className="block break-all rounded-lg bg-white p-3 text-xs leading-5 text-slate-700">
                    {data.keyPair.publicKey}
                  </code>
                ...
                <div className="rounded-xl border border-amber-200/70 bg-amber-50/50 p-4">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <Label>Private key preview</Label>
                  </div>
                  <pre className="overflow-hidden whitespace-pre-wrap rounded-lg bg-slate-900 p-3 text-xs leading-5 text-slate-400">
                    {data.keyPair.privateKeyMasked}
                  </pre>
                  <p className="mt-2 text-xs text-amber-700">
                    Only the opening and closing lines are shown. The complete
                    private key is never sent to the browser.
                  </p>
                </div>
              </div>
            ) : (
              <div className="mt-5 rounded-xl border border-dashed border-slate-300 bg-slate-50/50 p-6 text-center text-sm text-slate-500">
                No deployment key has been generated for <b>{data.primary}</b>.
              </div>
            )}
          </section>
          <section className={card}>
            <div className="flex items-center justify-between gap-3">
              <h2 className="font-bold">SSH users</h2>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setOpenForm(openForm === "ssh" ? null : "ssh")}
              >
            ...
            {(data.ssh ?? []).map((u) => (
              <div
                key={u}
                className="group -mx-2 flex justify-between rounded-lg border-t border-slate-100 px-2 py-3 transition-colors hover:bg-slate-50/50"
              >
                {u}
                <button
            ...
            {openForm === "ssh" && (
              <FormModal title="Add SSH user" onClose={() => setOpenForm(null)}>
                <form
                  className="space-y-3"
                  onSubmit={(e) => {
                    submit("add-ssh", e);
          ...
          <section className={card}>
            <div className="mb-3 flex items-center justify-between gap-3">
              <h2 className="font-bold">FTP users</h2>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setOpenForm(openForm === "ftp" ? null : "ftp")}
              >
                <Plus className="h-4 w-4" /> Add user
              </Button>
            </div>
            {(data.ftp ?? []).map((u) => (
              <div
                key={u.username}
                className="group -mx-2 flex justify-between rounded-lg border-b border-slate-100 px-2 py-3 transition-colors hover:bg-slate-50/50"
              >
                <div>
                  {u.username}
            ...
            {openForm === "ftp" && (
              <FormModal title="Add FTP user" onClose={() => setOpenForm(null)}>
                <form
                  className="space-y-3"
                  onSubmit={(e) => {
                    submit("add-ftp", e);
                ...
                >
                  ...
                  <Input
                    ...
                    defaultValue={`/home/${data.primary ?? ""}`}
                  />
                  <Button>
    ...
    if (section === "file-manager")
      return (
        <FileManager domain={domain} initialData={data as FileManagerData} />
      );
    ...
    if (section === "cron-jobs")
      return (
        <div className="grid gap-5">
          <section className={card}>
            <div className="mb-4 flex items-center justify-between gap-3">
              <h2 className="font-bold">Scheduled jobs</h2>
              <Button
                size="sm"
                onClick={() => setOpenForm(openForm === "cron" ? null : "cron")}
              >
            ...
            {((data.items as CronItem[]) ?? []).map((job) => (
              <div
                key={job.id}
                className="group -mx-2 flex justify-between rounded-lg border-b border-slate-100 px-2 py-4 transition-colors hover:bg-slate-50/50"
              >
                <div>
                  <code className="rounded bg-panel-50 px-1.5 py-0.5 text-xs text-panel-700">
                    {job.expression}
                  </code>
                  <p className="mt-1.5 text-sm">{job.command}</p>
                </div>
                <button
                  aria-label={`Delete cron job ${job.expression}`}
                  className="rounded-lg p-2 opacity-100 transition-opacity hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 sm:opacity-0 sm:group-focus-within:opacity-100 sm:group-hover:opacity-100"
                  onClick={() => act({ action: "delete", id: job.id })}
                >
                  <Trash2 className="h-4 w-4 text-red-500" />
                </button>
              </div>
          ...
          {openForm === "cron" && (
            <FormModal title="Add cron job" onClose={() => setOpenForm(null)}>
              <form
                className="space-y-4"
                onSubmit={(e) => {
                  submit("add", e);
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {[
                    ...
                    <button
                      key={value}
                      type="button"
                      className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium hover:bg-panel-50"
                      onClick={(event) => {
                ...
                />
                <p className="-mt-2 text-xs text-slate-500">
                  Enter one command per line. Commands run in order and stop if
                  one fails.
                </p>
                <div className="grid gap-2">
                  ...
                  {[
                    [
                      "Laravel scheduler",
                      `cd ${data.sitePath ?? "site"} && php artisan schedule:run`,
                    ],
                    [
                      "WordPress cron",
                      `cd ${data.sitePath ?? "site"} && wp cron event run --due-now`,
                    ],
                    [
                      "Node.js task",
                      `cd ${data.sitePath ?? "site"} && npm run cron`,
                    ],
                  ].map(([label, command]) => (
                    <button
                      key={label}
                      type="button"
                      className="hover:border-panel-300 rounded-lg border border-slate-200 bg-slate-50 p-3 text-left text-sm hover:bg-panel-50"
                      onClick={(event) => {
                        const form = event.currentTarget.closest("form");
                        const input = form?.elements.namedItem(
                          "command",
                    ...
                    >
                      <b className="block text-xs text-slate-700">{label}</b>
          ...
          <div className="space-y-2">
            {((data.items as string[]) ?? []).map((name) => (
              ...
              >
                <button
                  type="button"
                  className="min-w-0 flex-1 rounded-lg p-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panel-500"
          ...
          {logViewer &&
            createPortal(
              <div
                ...
                onMouseDown={(event) => {
                  if (event.target === event.currentTarget) setLogViewer(null);
                }}
              >
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-label={`Log file ${logViewer.name}`}
                  className="flex max-h-[calc(100vh-2rem)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"
                >
                  <div className="flex items-center justify-between border-b px-5 py-4">
                    <div>
                      <h3 className="font-bold">{logViewer.name}</h3>
                      <p className="text-xs text-slate-500">
                        {logViewer.truncated
                          ? "Showing the most recent 500 KB"
                          : "Complete log"}
                      </p>
                    </div>
                    <Button
                  ...
                  <pre className="min-h-[50vh] flex-1 overflow-auto bg-slate-950 p-5 font-mono text-xs leading-5 text-slate-200">
                    {logViewer.content || "This log is empty."}
                  </pre>
                </div>
              </div>,
              document.body,
