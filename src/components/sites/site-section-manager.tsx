...
export function SiteSectionManager({
      ...
      <div className={card}>
        <form onSubmit={(e) => submit("save", e)} className="space-y-4">
          ...
          <textarea
            ...
            className="min-h-[60vh] w-full resize-y rounded-xl bg-slate-950 p-5 font-mono text-xs leading-6 text-slate-100 outline-none ring-panel-500 focus:ring-2"
          />
          {feedback}
    ...
    return (
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        <section className={card}>
          <h2 className="mb-4 font-bold">Databases</h2>
  ...
  if (section === "certificates")
    return (
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        <section className={card}>
          <h2 className="mb-4 font-bold">Installed certificates</h2>
          <div className="space-y-3">
          ...
          <section key={x.key} className={card}>
            ...
            <div className="space-y-2">
              {((data[x.key] as string[] | undefined) ?? []).map((v) => (
                ...
                >
                  {v}
                  <button onClick={() => act({ action: x.del, value: v })}>
    ...
    return (
      <div className="grid gap-5 md:grid-cols-2">
        <section className={card}>
          ...
          {(data.ssh ?? []).map((u) => (
            <div key={u} className="flex justify-between border-t py-3">
              {u}
              <button
    ...
    return (
      <section className={card}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="font-bold">{data.path}</h2>
            <p className="text-sm text-slate-500">
              Site root files and folders
            </p>
          </div>
          <div className="flex gap-2">
            <form
              onSubmit={(e) => submit("new-file", e)}
              className="flex gap-2"
            >
              <Input name="name" placeholder="new-file.txt" required />
              <Button variant="outline">
                <FilePlus2 className="h-4 w-4" /> File
              </Button>
            </form>
            <form
              onSubmit={(e) => submit("new-folder", e)}
              className="flex gap-2"
            >
              <Input name="name" placeholder="folder" required />
              <Button variant="outline">
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {((data.items as string[]) ?? []).map((name) => (
            ...
            >
              ...
              <button onClick={() => act({ action: "delete", name })}>
                <Trash2 className="h-4 w-4 text-red-500" />
              </button>
            </div>
          ))}
  ...
  if (section === "cron-jobs")
    return (
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        <section className={card}>
          <h2 className="mb-4 font-bold">Scheduled jobs</h2>
          {((data.items as CronItem[]) ?? []).map((job) => (
            <div key={job.id} className="flex justify-between border-b py-4">
              <div>
                <code className="text-panel-700">{job.expression}</code>
                <p className="mt-1 text-sm">{job.command}</p>
              </div>
              <button onClick={() => act({ action: "delete", id: job.id })}>
                <Trash2 className="h-4 w-4 text-red-500" />
              </button>
            </div>
          ))}
