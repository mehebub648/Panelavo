...
export function UpdateManager({ initialState }: { initialState: UpdateState }) {
  ...
  async function call(url: string, init?: RequestInit) {
    ...
    if (!body.success)
      throw new Error(body.error?.message || "Update operation failed.");
    setState(body.data);
    return body.data as UpdateState;
  }
  ...
  async function save() {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Could not save repository.",
      );
    } finally {
      setBusy(null);
    }
  ...
  async function update() {
    ...
    try {
      ...
      toast.success(
        "Update started. Panelavo will reload when the staged build is ready.",
      );
    } catch (error) {
      toast.error(
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/60 px-5 py-4 sm:px-6">
        <div>
          <h3 className="font-bold">Panel updates</h3>
          <p className="text-sm text-slate-500">
            Stage, build, and deploy Panelavo without restarting hosted
            websites.
          </p>
        </div>
      ...
      <div className="space-y-4 p-5 sm:p-6">
        <div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Button
              variant="outline"
              disabled={!!busy || repository === state.repository}
              onClick={() => void save()}
            >
              {busy === "save" ? (
          ...
          <p className="mt-1.5 text-xs text-slate-500">
            HTTPS public Git repository, main branch. Changing it changes the
        ...
        <p className="text-xs text-slate-500">
          The update is built in a staging directory first. Runtime data and
          environment secrets are preserved. Panelavo reloads briefly; managed
          websites continue running.
        </p>
      </div>
    </section>
  );
