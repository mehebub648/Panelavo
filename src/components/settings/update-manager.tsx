...
export function UpdateManager({ initialState }: { initialState: UpdateState }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card">
    ...
    <div className="space-y-4 p-5 sm:p-6">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!!busy || running} onClick={() => void check()}>{busy === "check" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />} Check for updates</Button><Button disabled={!!busy || running || state.status === "current"} onClick={() => void update()}>{running || busy === "update" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <DownloadCloud className="h-4 w-4" />} {running ? "Updating…" : "Install latest"}</Button></div>
