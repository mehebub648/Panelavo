...
export function UserManager({
  ...
  async function act(body: Record<string, unknown>) {
    setBusy(true);
    try {
      const result = await fetch(`${apiBase}/api/users`, {
        method: "POST",
      ...
      const next = await fetch(`${apiBase}/api/users`).then((r) => r.json());
  ...
  return (
    <div className="animate-in fade-in mx-auto max-w-7xl space-y-8 duration-300">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl leading-relaxed text-slate-500">
            Manage super admins, managers, admins, and users. Admins create
            their own websites and only see sites assigned to them or created by
            them.
          </p>
        </div>
        ...
        <div className="divide-y divide-slate-100 md:hidden">
          {filteredUsers.map((user) => (
            <article key={user.id} className="p-4">
              <div className="flex items-start gap-3">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-panel-100 font-bold uppercase text-panel-700 ring-1 ring-panel-500/20">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-1.5">
                    ...
                    {user.status === false ? (
                      <span className="rounded-full bg-red-50 px-2 py-0.5 text-[11px] font-medium text-red-600 ring-1 ring-red-600/20">
                        Disabled
                      </span>
                    ) : (
                      <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-600 ring-1 ring-emerald-600/20">
                        Active
                      </span>
                    )}
          ...
          {filteredUsers.length === 0 && (
            <div className="px-6 py-12 text-center text-slate-500">
              <UserRound className="mx-auto mb-3 h-8 w-8 opacity-20" />
              ...
              {filteredUsers.map((user) => (
                ...
                >
                    ...
                    <div className="flex flex-col items-start gap-1.5">
                      ...
                      <span className="flex items-center gap-1 text-xs text-slate-500">
                        ...
                        user.panelRole === "manager"
                          ? "All sites"
                          : user.sites?.length
                            ? `${user.sites.length} assigned sites${user.panelRole === "admin" ? " + own" : ""}`
                            : user.panelRole === "admin"
                              ? "Own created sites"
                              : "No sites assigned"}
                      </span>
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    {user.status === false ? (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-2 py-1 text-xs font-medium text-red-600 ring-1 ring-red-600/20">
                        Disabled
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-600 ring-1 ring-emerald-600/20">
                ...
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center justify-center">
                      <UserRound className="mb-3 h-8 w-8 opacity-20" />
                      <p>No users found matching your search</p>
                    </div>
                  </td>
                </tr>
...
function AddUserForm({
  ...
  if (inviteUrl)
    return (
      <UserModal title="Invitation created" close={close}>
        <div className="flex-1 overflow-y-auto bg-slate-50/50 px-6 py-8 md:px-8">
          <div className="mx-auto max-w-lg text-center">
            ...
            <h4 className="mt-5 text-xl font-bold text-slate-900">
              Share this link with {firstName || username}
            </h4>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              The account will be created with the access you defined once they
              open the link and choose their own password. Nothing is stored
              until then.
            </p>
            <div className="mt-6 flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <code className="min-w-0 flex-1 truncate text-left text-xs text-slate-600">
                {inviteUrl}
              </code>
              <Button
                type="button"
            ...
            <p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-slate-400">
              <Clock className="h-3.5 w-3.5" /> The link expires in 24 hours and
              can only be used once.
            </p>
          </div>
        </div>
        <div className="flex shrink-0 justify-end border-t bg-white p-4 md:px-8">
  ...
  return (
    <UserModal title="Create New User" close={close}>
      ...
      >
        <div className="flex-1 space-y-8 overflow-y-auto bg-slate-50/50 px-6 py-6 md:px-8">
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_2fr]">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Credentials used to log into this server.
              </p>
            </div>
            <div className="space-y-4 rounded-xl border bg-white p-5 shadow-sm">
              <div className="space-y-1">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {allowInvites && (
                    ...
                    >
                      <span className="flex items-center gap-2 font-semibold text-slate-800">
                        <Lock className="h-4 w-4 text-panel-600" /> Set password
                        now
                      </span>
                  ...
                  >
                    <span className="flex items-center gap-2 font-semibold text-slate-800">
                      <LinkIcon className="h-4 w-4 text-panel-600" /> Send an
                      invite link
                    </span>
                    <span className="mt-1 block text-xs text-slate-500">
                      The user sets their own password via a 24-hour link.
                    </span>
                  </button>
                </div>
              </div>
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_2fr]">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Determine what this user can see and do.
              </p>
            </div>
            <div className="space-y-5 rounded-xl border bg-white p-5 shadow-sm">
              <div className="space-y-1">
                ...
                >
                  ...
                  <option value="admin">
                    Admin (Creates own websites + assigned sites)
                  </option>
              ...
              {(role === "user" || role === "admin") && (
                <div className="space-y-3 border-t border-slate-100 pt-2">
                  ...
                  <div className="max-h-48 space-y-1 overflow-y-auto rounded-lg border bg-slate-50 p-2">
                    {sites.length === 0 ? (
                      ...
                      sites.map((site) => (
                        <label
                          key={site}
                          className="flex cursor-pointer items-center gap-3 rounded-md border border-transparent bg-white p-2.5 transition-colors hover:border-slate-200 hover:bg-slate-200/50"
                        >
                          <input
                            type="checkbox"
        ...
        <div className="flex shrink-0 justify-end gap-3 border-t bg-white p-4 md:px-8">
        ...
        <div className="flex-1 space-y-8 overflow-y-auto bg-slate-50/50 px-6 py-6 md:px-8">
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_2fr]">
            ...
            <div className="rounded-xl border bg-white p-5 shadow-sm">
              <label
                className={`flex items-center justify-between rounded-xl border p-4 ${status ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"} cursor-pointer transition-colors`}
              >
                <div>
                  <div
                    className={`font-bold ${status ? "text-emerald-900" : "text-red-900"}`}
                  >
                    Account {status ? "Active" : "Disabled"}
                  </div>
                  <div
                    className={`mt-1 text-xs ${status ? "text-emerald-700" : "text-red-700"}`}
                  >
                    {status
                      ? "The user can log in and access their resources."
...
export function UserModal({
  ...
  return createPortal(
    <div
      className="animate-in fade-in fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/40 p-3 backdrop-blur-sm duration-200 sm:p-6"
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        tabIndex={-1}
        className="animate-in zoom-in-95 flex max-h-[calc(100dvh-1.5rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl duration-200 sm:max-h-[calc(100dvh-3rem)]"
      >
