...
CloudPanel's configured site type and serving/document root are authoritative for web traffic. Panelavo stores a separate per-site application root in `.data/site-roots.json`; Git, File Manager, Terminal defaults, Environment, Operations, cron working-directory guidance, and backups use that root, falling back to CloudPanel's serving root when no override exists. Both roots must remain inside the site's `htdocs` folder. Detection, runtime/tool availability, permissions, manifest validation, and safety policy are separate preflight results; a manifest alone must never make an action runnable. Operations plans are server-owned, execute exact argument arrays without a shell, stay inside the configured application root, and take a per-site lock. Missing tools block visibly.
