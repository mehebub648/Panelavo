...
The CloudPanel site user must retain recursive read/write/traverse access to its configured application root even when containers or other root-managed runtimes own files there. Enforce this with a named POSIX ACL for the site user plus default ACLs on every directory so future descendants inherit access. Do not solve ownership drift with world-writable modes, broad ownership replacement, Docker-group membership, or traversal outside `/home/<site-user>/htdocs`.
