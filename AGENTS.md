...
Rootful Docker Compose actions are Super Admin-only and must use the explicitly selected Compose file and project name after CLI, Compose plugin, daemon, configuration, and host-safety checks. Do not install Docker automatically or add Panelavo/site users to the `docker` group. Keep Operations execution synchronous and bounded; do not claim atomic releases, automatic database rollback, recursive workspace discovery, or a guessed static build output.
