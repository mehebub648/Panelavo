...
Broker protocol 23 adds the production database gateway, host maintenance inventory, automatic disk-pressure cleanup, and the direct-`clpctlWrapper` denial boundary. Trusted `sudo bash setup.sh` is mandatory before deploying this release: setup first refuses active non-loopback database clients, makes ports 3306/33060 loopback-only, installs the dedicated ProxySQL and Nginx Stream layers, prepares firewall rules for managed ports 43000–43255, and validates the complete fail-closed broker health contract. Supply `DATABASE_GATEWAY_CERTIFICATE_FILE`, `DATABASE_GATEWAY_PRIVATE_KEY_FILE`, and optionally `DATABASE_GATEWAY_CA_FILE` for a publicly trusted wildcard identity; otherwise the installer creates a private Panelavo client CA. The ordinary updater cannot perform these root-owned changes.
