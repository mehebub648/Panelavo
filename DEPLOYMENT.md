...
Site-scoped fixes edit only files inside the site root and need the ordinary Operations permission. When a Compose project fails the host-safety policy solely because a published port is not bound to the loopback interface, the "Host safety policy" check offers a fix that rewrites the Compose file so each published port binds to `127.0.0.1` — the address the site's reverse proxy already targets — keeping the same port numbers. The rewrite is validated with `docker compose config` and re-scanned against the full safety policy, and is committed only when the result is fully safe; the original file is backed up as `<file>.panelavo.bak`. If a project also uses a feature only an operator can resolve (privileged mode, host namespaces, out-of-root mounts, and so on), or uses a port form Panelavo will not rewrite textually, no automatic fix is offered and the remediation guidance stands.
