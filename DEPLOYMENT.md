...
`setup.sh` installs the host `acl` utility used by the root-owned broker to preserve site-user access across runtime ownership changes. The broker confines enforcement to the configured root under `/home/<site-user>/htdocs`, adds a named `rwX` ACL without replacing ownership, and applies default directory ACLs for future descendants. Existing installations must run the trusted `setup.sh` once when upgrading to broker protocol 2; the ordinary in-panel updater intentionally cannot install host packages or replace the root-owned broker.
