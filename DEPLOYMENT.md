...
Fleet Hub is application protocol 1 and requires no `setup.sh` or broker change beyond the already-required protocol 24 installation. Every participating panel must run Panelavo 0.1.115 or newer on a publicly resolvable hostname with a valid certificate on HTTPS port 443. Custom ports, self-signed certificates, private-only addresses, tunnels, multiple Hubs, and Hub chaining are intentionally rejected. Permit outbound DNS plus HTTPS/443 between the Hub and Nodes; Nodes need no new inbound port because they reuse the existing Panelavo vhost and initiate enrollment.

Fleet identity, encrypted private keys, connections, replay records, cached health, and rolling-update journals live in `.data/fleet-state.enc.json`; normal deployment and self-update preservation of `.data` is therefore mandatory. Keep a single PM2 process. Disconnecting or losing the Hub never stops a Node's local panel, websites, users, or services. A rolling update changes one selected Node at a time and stops on the first failure; after resolving that Node, revisit Fleet to resume or start the remaining selection. Fleet does not distribute repository configuration, broker upgrades, credentials, or large artifacts.
